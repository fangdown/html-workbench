...
import { MODEL_GROUPS, type BrowserModelConfig, type BrowserRun, type ModelConfig, type ModelInput, type RunDetail, type RunPage, type RunStatus, type RunSummary } from '../shared/types';
...
function editConfig(config?: ModelConfig) {
  ...
  const nextGroup = MODEL_GROUPS.find(group => !configs.value.some(item => item.group === group)) ?? MODEL_GROUPS[0];
  configForm.value = config
    ? { group: config.group, baseUrl: config.baseUrl, model: config.model, protocol: config.protocol, stream: config.stream, isDefault: config.isDefault, apiKey: '' }
...
function saveConfig() {
  ...
  try {
    const existingGroup = configs.value.find(item => item.group === configForm.value.group);
    const saved = saveLocalModel(configForm.value, editingId.value ?? existingGroup?.id ?? null);
    loadConfigs();
...
<template>
  <div class="app-shell">
    ...
    <div v-if="showSettings" class="modal-backdrop"><section class="settings-modal card"><div class="modal-heading"><div><span class="eyebrow">仅保存在当前浏览器 · 不共享</span><h2>{{ editingId ? '编辑模型' : '添加模型' }}</h2></div><button class="modal-close" @click="showSettings = false">×</button></div><div class="settings-form"><label>模型分组<select v-model="configForm.group"><option v-for="group in MODEL_GROUPS" :key="group" :value="group">{{ group }}</option></select></label><label>Base URL<input v-model="configForm.baseUrl" placeholder="https://api.opens.chat/v1" /></label><label>模型名称<input v-model="configForm.model" placeholder="gpt-6-astra" /></label><label>API Key <small v-if="editingId">留空以保留当前 Key</small><input v-model="configForm.apiKey" type="password" autocomplete="new-password" placeholder="不会回显已保存的 Key" /></label><label>接口协议<select v-model="configForm.protocol"><option value="chat-completions">Chat Completions</option><option value="responses">Responses</option><option value="anthropic-messages">Claude Messages</option></select></label><div class="toggle-row"><label class="check-label"><input v-model="configForm.stream" type="checkbox" /> 流式输出</label><label class="check-label"><input v-model="configForm.isDefault" type="checkbox" /> 设为默认模型</label></div></div><div class="modal-footer"><p class="local-config-note">地址、Key 和模型配置仅保存在当前浏览器；生成时临时发送至后端调用，作品保存在服务器并共享。换浏览器或清除网站数据后需重新配置。</p><div class="config-list"><div v-for="config in configs" :key="config.id" class="config-line"><span>{{ config.group }} <small>{{ config.hasKey ? config.keyMask : 'Key 不可用' }}</small></span><span><button @click="editConfig(config)">编辑</button><button @click="removeConfig(config)">删除</button></span></div></div><div class="modal-actions"><button class="outline-button" @click="showSettings = false">取消</button><button class="primary-button" :disabled="savingConfig" @click="saveConfig">{{ savingConfig ? '保存中…' : '保存配置' }} <b>↗</b></button></div></div></section></div>
