...
const configForm = ref<ModelInput>({ name: '', group: MODEL_GROUPS[0], baseUrl: 'https://api.opens.chat/v1', model: 'gpt-6-astra', protocol: 'responses', stream: true, isDefault: true, apiKey: '' });
...
function editConfig(config?: ModelConfig) { editingId.value = config?.id ?? null; configForm.value = config ? { name: config.name, group: config.group, baseUrl: config.baseUrl, model: config.model, protocol: config.protocol, stream: config.stream, isDefault: config.isDefault, apiKey: '' } : { name: '', group: MODEL_GROUPS[0], baseUrl: 'https://api.opens.chat/v1', model: 'gpt-6-astra', protocol: 'responses', stream: true, isDefault: configs.value.length === 0, apiKey: '' }; showSettings.value = true; }
function saveConfig() {
